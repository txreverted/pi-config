...
This private Pi package adds interactive tools, prompt templates, and fixed system-prompt policies. Truncated web output keeps a full copy in a temporary directory.
...
- `web_search` and `web_fetch` send queries and URLs to Firecrawl and may use Firecrawl credits. They read `FIRECRAWL_API_KEY` from the environment and use keyless access when Firecrawl permits it.
...
- [`extensions/web.ts`](extensions/web.ts) provides Firecrawl-backed `web_search` and `web_fetch`. Search accepts 500 characters, 1-10 results, and 10 domains per filter. Tool output stops at 2,000 lines or 50KB. Firecrawl responses stop at 10MB. See the [web tests](https://github.com/txreverted/pi-config/blob/main/test/web-core.test.mjs).
- [`extensions/ui.ts`](extensions/ui.ts) adds a compact live footer and a single-row working loader. The footer caches cumulative usage, shows last-run time, and keeps Pi's context, model, branch, session, and extension status data. Loader color follows the current thinking level. Pi does not expose startup resource rendering, so the built-in `[Extensions]` heading keeps the theme's `mdHeading` color. See the [UI tests](https://github.com/txreverted/pi-config/blob/main/test/ui-extension.test.mjs).
...
- [`/R-DOCS [scope]`](prompts/R-DOCS.md) rebuilds docs, including replacing dirty in-scope docs without confirmation. [`/R-GIT`](prompts/R-GIT.md) merges green PRs without confirmation. [`/R-IMPL [scope]`](prompts/R-IMPL.md) audits without editing unless asked. Their prompt expansions combine to at most 775 tokens. See the [config tests](https://github.com/txreverted/pi-config/blob/main/test/config.test.mjs).
...
Local adaptations keep Ponytail at fixed full strength, stop Unslop from inventing personality, and extend Caveman from replies to all non-code output.
