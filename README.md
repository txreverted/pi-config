...
- `web_search` and `web_fetch` send queries and URLs to Firecrawl. When `FIRECRAWL_API_KEY` is absent or blank, they use [Firecrawl Keyless](https://www.firecrawl.dev/blog/firecrawl-keyless-launch). A supplied key uses your account.
- When Keyless credits are unavailable or exhausted, requests fail. Set `FIRECRAWL_API_KEY` and restart Pi to continue with your account. Invalid or exhausted keys do not fall back to Keyless.

## Change

- [`extensions/ask.ts`](extensions/ask.ts) provides `ask_user_question` in TUI and RPC sessions. It accepts 1-4 questions with 2-4 choices. `Other` answers are normalized to one line and stop at 2,000 UTF-8 bytes. Review entries show answer previews of at most 160 characters; full answers remain in tool results. Its metadata estimate is at most 400 tokens. See the [ask tests](https://github.com/txreverted/pi-config/blob/main/test/ask-extension.test.mjs).
- [`extensions/web.ts`](extensions/web.ts) provides Firecrawl-backed `web_search` and `web_fetch`. Search accepts 500 characters, 1-10 results, 10 domains per filter, and the `developer`, `research`, and `pdf` categories. Fetch rejects local, private, and recognized signed URLs before forwarding. Tool output stops at 2,000 lines or 50KB. See the [web tests](https://github.com/txreverted/pi-config/blob/main/test/web-core.test.mjs).
...
- [`extensions/ponytail.ts`](extensions/ponytail.ts) and [`extensions/unslop.ts`](extensions/unslop.ts) append fixed policies on every agent run. Ponytail controls implementation scope. Unslop keeps chat and prose artifacts concise, natural, and grammatical while preserving required detail. Unslop is at most 600 estimated tokens; the combined policy estimate is at most 1,450 tokens. Runtime prose policy: [`policies/UNSLOP.md`](policies/UNSLOP.md). See the [policy tests](https://github.com/txreverted/pi-config/blob/main/test/policies.test.mjs).
- [`/r-docs-rebuild [scope]`](prompts/r-docs-rebuild.md) rebuilds docs, including replacing dirty in-scope docs without confirmation. [`/r-ship`](prompts/r-ship.md) merges green PRs without confirmation, then removes clean branches and worktrees it created. [`/r-audit [scope]`](prompts/r-audit.md) audits without editing unless asked. Their prompt expansions combine to at most 830 tokens. See the [config tests](https://github.com/txreverted/pi-config/blob/main/test/config.test.mjs).

Policy sources: [Ponytail](https://github.com/DietrichGebert/ponytail/blob/2ed6c52c9d7e5e56942508591085fd45dea277d3/skills/ponytail/SKILL.md), [Unslop](https://github.com/cursor/plugins/blob/99559f2f52047978602ef365589275831e76af07/pstack/skills/unslop/SKILL.md), and [Caveman](https://github.com/JuliusBrussee/caveman/blob/2f49f0e1a352aa810e70056b7930aeb0b3d219b4/src/rules/caveman-activate.md). Local adaptations keep Ponytail at fixed full strength and combine Unslop's editorial guidance with Caveman's brevity guidance in one grammatical prose policy. Notices: [`ponytail.LICENSE`](policies/ponytail.LICENSE), [`unslop.LICENSE`](policies/unslop.LICENSE), and [`caveman.LICENSE`](policies/caveman.LICENSE).
