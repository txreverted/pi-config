...
- [`extensions/ui.ts`](extensions/ui.ts) adds a compact live footer and a single-row working loader with elapsed time. The footer caches cumulative usage and keeps Pi's context, model, branch, session, and extension status data. Spinner color follows the current thinking level. Pi does not expose startup resource rendering, so the built-in `[Extensions]` heading keeps the theme's `mdHeading` color. See the [UI tests](https://github.com/txreverted/pi-config/blob/main/test/ui-extension.test.mjs).
