...
export default function subagentToolsExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "git_status",
    label: "git status",
    description: "Show the current repository branch and concise working-tree status without modifying the repository.",
  ...
  pi.registerTool({
    name: "git_diff",
    label: "git diff",
    description: "Inspect unstaged or staged working-tree changes without modifying the repository. Optional paths are passed after '--'.",
    parameters: Type.Object({
